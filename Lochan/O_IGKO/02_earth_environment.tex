...
\begin{tikzpicture}
   ...
   % Draw top circles with labels inside
   \draw (0,0) circle (\radius) node {\parbox{2.7cm}{\raggedright \large Endangered species}};
   \draw (\shift*\radius,0) circle (\radius) node {\parbox{2.7cm}{\raggedleft \large Endemic to temperate forests of Himalayas}};
   
   % Draw bottom circle with label inside
   \draw (\shift*\radius/2,-\shift*\radius) circle (\radius) node {\parbox{2.7cm}{\centering \large Extinct species}};

   % Label the intersection of the top two circles
   \node at (\shift*\radius/2, 0) {\Large X};
\end{tikzpicture}
}
...
\question Which of the following is \textbf{NOT} a part of Pachmarhi Biosphere Reserve?
    ...
    \begin{figure}[h!]
        ...
        \includegraphics[width=3cm]{01.jpg}
      \end{figure}
